...
entity countery_cas is
    ...
    port (
        ...
        switch : in std_logic;                         --! Clock enable input
...
end entity countery_cas;

-------------------------------------------------

architecture behavioral of countery_cas is
...
begin
    ...
    begin
        if rising_edge(clk) then
            ...
            if rst = '1' then    -- Synchronous, active-high reset
            ...
            elsif en = '1' and switch2 = '0' then  -- Clock enable activated
                if sig_cnt2mj = MJ_MAX then
                ...
                if sig_cnt2md = MD_MAX then
                    if sig_cnt2mj = MJ_MAX then
                        sig_cnt2md <= 0; 
                        sig_cnt2hj <= sig_cnt2hj + 1;
                    end if;
                end if;
                if sig_cnt2hj = HJ_MAX then
                    if sig_cnt2md = MD_MAX then
                        if sig_cnt2mj = MJ_MAX then      
                            sig_cnt2hj <= 0;
                ...
                if sig_cnt2hd=2 AND sig_cnt2hj=3 AND sig_cnt2md=5 AND sig_cnt2mj=9 then
                    sig_cnt2hd <= 0;
                    sig_cnt2hj <= 0;
                    sig_cnt2md <= 0;
                    sig_cnt2mj <= 0;
            ...
            elsif switch2 = '1' and en_button = '1' then
               if switch = '0' then
